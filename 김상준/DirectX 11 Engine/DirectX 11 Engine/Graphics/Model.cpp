...
void Model::Draw(const XMMATRIX& worldMatrix, const XMMATRIX& viewProjectionMatrix)
{
	...
	{
		this->cb_vs_vertexshader->data.mat = meshes[i].GetTransformMatrix() * worldMatrix * viewProjectionMatrix;
...
std::vector<Texture> Model::LoadMaterialTextures(aiMaterial* pMaterial, aiTextureType textureType, const aiScene* pScene)
{
	...
	TextureStorageType storetype = TextureStorageType::Invalid; // setting default value
	unsigned int textureCount = pMaterial->GetTextureCount(textureType);

	if (textureCount == 0) // there are no textures
	{
		...
		case aiTextureType_DIFFUSE:
			...
			if (aiColor.IsBlack()) // If color = black, just make it gray
			{
